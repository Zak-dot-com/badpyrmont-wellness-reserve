...
// Sample detailed descriptions for each addon type
const addonDetailedDescriptions = {
  "massage-therapy": {
...
// Get detailed description or fallback to a generic one
const getAddonDetails = (itemId: string) => {
  return addonDetailedDescriptions[itemId as keyof typeof addonDetailedDescriptions] || {
...
const AddonDetailDialog = ({
  ...
  const details = getAddonDetails(itemId);
          ...
          <div className="flex items-center gap-4">
            {selected && (
              <div className="flex items-center space-x-2">
                ...
                <span className="text-amber-700 font-medium w-6 text-center">{quantity}</span>
                <Button 
                  variant="outline" 
                  size="icon"
                  className="h-8 w-8 rounded-full"
            ...
            <Button 
              onClick={toggleAddOn}
              className={selected ? "bg-gray-200 hover:bg-gray-300 text-gray-800" : "bg-amber-500 hover:bg-amber-600"}
            >
                  ...
                  <CardFooter className="bg-gray-50 flex justify-between items-center flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <AddonDetailDialog 
                        ...
                        toggleAddOn={() => toggleAddOn(category.id, item.id)}
                        incrementQuantity={() => incrementQuantity(category.id, item.id, item.quantity)}
                        decrementQuantity={() => decrementQuantity(category.id, item.id, item.quantity)}
                      />
